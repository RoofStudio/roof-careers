...
const ApplicationForm: React.FC<ApplicationFormProps> = ({ onSuccess }) => {
  ...
  const toggleTool = (id: string) => {
    setSelected((prev) => {
      ...
      // Start at the honest middle: one click to adjust, never a blank level
  ...
  const setLevel = (id: string, level: Level) =>
    setSelected((prev) => (prev[id] ? { ...prev, [id]: level } : prev))
  ...
  return (
    <section id="form" className="wide-column scroll-mt-20 pb-24">
      <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-8">
        ...
        >
          <p className="label mb-3">{t("form.label")}</p>
        ...
        >
          <div className="text-column mb-6 flex flex-col gap-2 text-center">
            ...
            <div className="mt-2 flex items-center justify-center gap-3">
              ...
              >
                {t("tools.selectedCount", { count: selectedCount, total: TOTAL_TOOLS })}
              </span>
              {selectedCount > 0 && (
                <button
                  type="button"
          ...
          <ToolPicker selected={selected} onToggle={toggleTool} onLevel={setLevel} />
